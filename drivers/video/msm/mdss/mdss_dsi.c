...
/* [PLATFORM]-Mod-BEGIN by TCTNB.CY, PR-789023, 2014/11/20, add power IC  */
#ifdef CONFIG_TCT_8X16_IDOL3
#include <linux/i2c.h>


static int gpio_lcd_pwr_enp = -1;
static int gpio_lcd_pwr_enn = -1;
static int tps65132b_init(void)
{
	int err = 0;

	if (gpio_is_valid(gpio_lcd_pwr_enp)) {
		err = gpio_request(gpio_lcd_pwr_enp, "display_pwron_gpiop");
		if (err) {
			pr_err("%s: display_pwron_gpiop request failed\n", __func__);
			goto err_gpio_enp;
		}

		err = gpio_direction_output(gpio_lcd_pwr_enp, 1);
		if (err) {
			pr_err("%s: set_direction for display_pwron_gpiop failed\n", __func__);
			goto err_gpio_enp;
		}
	}

	if (gpio_is_valid(gpio_lcd_pwr_enn)) {
		err = gpio_request(gpio_lcd_pwr_enn, "display_pwron_gpion");
		if (err) {
			pr_err("%s: display_pwron_gpion request failed\n", __func__);
			goto err_gpio_enn;
		}

		err = gpio_direction_output(gpio_lcd_pwr_enn, 1);
		if (err) {
			pr_err("%s: set_direction for display_pwron_gpion failed\n", __func__);
			goto err_gpio_enn;
		}
	}
...
err_gpio_enn:
	gpio_free(gpio_lcd_pwr_enn);
err_gpio_enp:
	gpio_free(gpio_lcd_pwr_enp);
	return err;
}


static int set_power_ic_gpio_on(void)
{
	int err;
...
static int set_power_ic_gpio_off(void)
{
	int err;

	mdelay(5);
	err = gpio_direction_output(gpio_lcd_pwr_enp, 0);
	mdelay(10);
	if (err) {
		pr_err("%s: set_direction for display_pwron_gpiop failed\n", __func__);
		goto err_gpio_enp;
	...
	err = gpio_direction_output(gpio_lcd_pwr_enn, 0);
	mdelay(10);
	if (err) {
		pr_err("%s: set_direction for display_pwron_gpion failed\n", __func__);
		goto err_gpio_enn;
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.CY, PR-789023, 2014/11/20,  r63315 lcd  */
//#ifdef CONFIG_TCT_8X16_IDOL3
//	set_power_ic_gpio_on();
//#endif
/* [PLATFORM]-Mod-END by TCTNB.CY*/
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.CY, 2014/11/20,add lcd power IC control*/
#ifdef CONFIG_TCT_8X16_IDOL3
	gpio_lcd_pwr_enp = of_get_named_gpio(ctrl_pdev->dev.of_node,
		 "qcom,platform-lcd-power-gpio-enp", 0);

	gpio_lcd_pwr_enn = of_get_named_gpio(ctrl_pdev->dev.of_node,
		 "qcom,platform-lcd-power-gpio-enn", 0);

	rc = tps65132b_init();
	if (rc)
		pr_err("%s: dsi panel powerIC control failed ret=%d \n", __func__, rc);
