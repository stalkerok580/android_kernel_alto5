...
/* [PLATFORM]-Mod-BEGIN by TCTNB.CY, PR-955026, 2015/4/10, add qualcomm patch for blue screen during charging*/
...
#ifndef CONFIG_TCT_8X16_ALTO5_PREMIUM
	ret = mdss_mdp_overlay_get_fb_pipe(mfd, &pipe,
					MDSS_MDP_MIXER_MUX_LEFT);
	if (ret) {
		pr_err("unable to allocate base pipe\n");
...
#endif
/* [PLATFORM]-Mod-END by TCTNB.CY, 2015/4/10*/
	buf = &pipe->back_buf;
	if (mdata->mdss_util->iommu_attached()) {
		if (!mfd->iova) {
